    ...
    return (
        <>
            {datas.map((movie, index) => (
                <div className="col-2 trendingMovie" style={{ backgroundImage: `url(${getImage(movie.poster_path)})`, width:"270px", height:"370px", backgroundSize:"cover", marginRight:"20px", position:"relative" }} key={index} data-bs-toggle="modal" data-bs-target={"#a" + index + movie.id}>
                    <h1><span style={{fontSize:"200px", marginLeft:"-20px", position:"absolute",bottom:"-60px" }}>{index + 1}</span></h1>
                        ...
                        <div className="modal-dialog modal-xl">
                            <div className="modal-content">
                                <div className="modal-header" style={{ backgroundImage: `url(${getImage(movie.poster_path)})`, height: "50vh", backgroundSize: "cover", position: "relative" }}>
                                    <h2 className="modal-title p-3" id={"label" + index + movie.id}>{movie.original_title}</h2>
